...
int main()
{
    ...
    auto x = std::make_shared<Value>(1);
    auto y = std::make_shared<Value>(3);
    auto z = std::make_shared<Value>(2);

    std::vector<std::shared_ptr<Value>> v = { x, y, z };
    Perceptron p(3);
    std::cout << p.predictNext(v) << std::endl;
    auto ve = p.parameters();
    for (auto asdf : ve)
    {
        std::cout << asdf << " ";
    }
